...
public class ValueVisitor extends AbstractCGVisitor<Void, Void> {
	...
	//VAR
		...
		public Void visit(Variable var, Void params) {
			if (var.getType() instanceof StructType) {
				for (int i = ((StructType) var.getType()).getList().size() - 1; i >= 0; i--) {
					var.accept(this.addressVisitor, params);
					this.cg.pushi(((StructType) var.getType()).getList().get(i).getOffset());
					this.cg.addi();
					this.cg.load(((StructType) var.getType()).getList().get(i).getType());
				}
			} else if (var.getType() instanceof ArrayType) {
				for (int i = ((ArrayType) var.getType()).getSize() - 1; i >= 0; i--) {
					ArrayAccess a = new ArrayAccess(var.getLine(),var.getColumn(),new IntLit(var.getLine(),var.getColumn(),i), var);
					a.setType(((ArrayType) var.getType()).getInnerType());
