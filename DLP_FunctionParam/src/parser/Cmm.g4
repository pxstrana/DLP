...
type returns [Type ast]: builtInType           {$ast= $builtInType.ast;} 
	...
	| n='Func'	'<' r=type {List<VariableDefinition> params= new ArrayList<>();}
	
	(c=',' t1=type {
		VariableDefinition var1=new VariableDefinition($c.getLine(),$c.getCharPositionInLine()+1,"",$t1.ast);
		params.add(var1 ); } )* '>'	 
	  {$ast=new FunctionType($n.getLine(),$n.getCharPositionInLine()+1,$r.ast,params);}
