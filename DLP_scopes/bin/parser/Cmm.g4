...
expressionList returns [List<Expression> ast = new ArrayList<Expression>();]:
		...
		;
block returns[List<Statement> ast=new ArrayList<Statement>();]: type  statement {$ast.add($statement.ast);}
  		|  '{'  (m=multipleVarDef {$ast.addAll($m.ast);}) ?  (statement {$ast.add($statement.ast);} )* '}' 
  		;
