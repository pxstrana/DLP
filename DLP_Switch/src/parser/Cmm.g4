...
arguments returns [List<VariableDefinition> ast=new ArrayList<VariableDefinition>();]: 
	(      builtInType n=ID  {$ast.add(new VariableDefinition($n.getLine(),$n.getCharPositionInLine()+1,$n.text,$builtInType.ast));} 
	(',' b=builtInType i=ID  {$ast.add(new VariableDefinition($i.getLine(),$i.getCharPositionInLine()+1,$i.text,$b.ast));})*)?
	;
...
statement returns [Statement ast]: 
		...
		| n='switch' '(' e=expression ')' '{' {Switch swi=new Switch($n.getLine(),$n.getCharPositionInLine()+1,$e.ast);}
		
			('case' e1=expression ':' 
			
			{List<Statement> listStmt= new ArrayList<>();}(st=statement {listStmt.add($st.ast);} )* 'break;'
			
			 { swi.addCase($e1.ast,listStmt); }  )+   {$ast = swi;}
