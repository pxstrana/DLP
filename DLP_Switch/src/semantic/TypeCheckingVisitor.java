...
public class TypeCheckingVisitor extends AbstractVisitor<Void, Type> {
	...
	public Void visit(FunctionInvocation i, Type t) {
		...
		i.setType(i.getVariable().getType().functionInvocation( 
				i.getParameters().stream().map(e -> e.getType()).collect(Collectors.toList()), i));
		return null;
	}
	...
	public Void visit(UnaryNegation unaryNegation, Type param) {
		...
		unaryNegation.setType(unaryNegation.getExpression().getType().unaryNegation(unaryNegation));	
		return null;
	}
	...
	public Void visit(FieldAccess fa, Type t) {
		super.visit(fa, t);
		fa.setType(fa.getExpression().getType().fieldAccess(fa.getName(), fa));
		return null;
	}
	...
	public Void visit(Variable c, Type t) {
		super.visit(c, t);		
		 if (c.getDefinition() == null) {
	           ...
	           c.setType(new ErrorType(c.getLine(),c.getColumn(),  "Variable has no definition"));
	        } else {
	...
	@Override
	public Void visit(Switch s, Type params) {
		super.visit(s, params);
		if(!(s.getCondition().getType().isLogical()))
			new ErrorType(s.getLine(), s.getColumn(), "La condicion ha de ser logica");
		for (Case case1 : s.getCasesList()) {
			if(!(case1.getCondition().getType().isLogical()))
				new ErrorType(s.getLine(), s.getColumn(), "El caso ha de ser logico");
