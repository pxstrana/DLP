...
arguments returns [List<VariableDefinition> ast=new ArrayList<VariableDefinition>();]: 
	(     b=type n=ID  {$ast.add(new VariableDefinition($n.getLine(),$n.getCharPositionInLine()+1,$n.text,$b.ast,true));} 
	(',' b=type i=ID  {$ast.add(new VariableDefinition($i.getLine(),$i.getCharPositionInLine()+1,$i.text,$b.ast,true));})*)?
	;
