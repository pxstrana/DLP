...
public class AdressVisitor extends AbstractCGVisitor<Void,Void> {
	...
	public Void visit (Variable f, Void param) {
		if(f.getDefinition().getScope() == 0) {
			...
			cg.pushi(((VariableDefinition)f.getDefinition()).getOffset()); //offset respecto bp
			cg.add(IntType.getInstance());//push la direccion local
			if((f.getType() instanceof StructType || f.getType() instanceof ArrayType) && ((VariableDefinition)f.getDefinition()).isParam()) {
				this.cg.load(new IntType());
			}
		}
		return null;
	} 
